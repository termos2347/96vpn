...
async def create_client(user_id: int, email: str, username: str, session: AsyncSession) -> Client:
...
async def update_client(client_id: int, email: str, username: str, session: AsyncSession) -> bool:
    async with AsyncSessionLocal() as session:
        try:
...
async def get_or_create_user(user_id: int, username: str = None, session: AsyncSession = None):
...
async def set_vpn_client_id(user_id: int, client_id: str, session: AsyncSession = None) -> bool:
...
async def delete_client(client_id: int, session: AsyncSession) -> bool:
    async with AsyncSessionLocal() as session:
        try:
